<script lang="ts">
	...
	onMount(async () => {
		// Attempt instant hydration from localStorage if store is not populated yet
		if (!$user && typeof window !== 'undefined' && localStorage.getItem('aria_user')) {
			try {
				const savedUser = JSON.parse(localStorage.getItem('aria_user') || '');
				if (savedUser) {
					if (savedUser.id === 'QH8wKG8nWZVtUQEy2pppuBuNZgC3' || savedUser.email === 'mrpinpinpro@gmail.com') {
		...
		// Wait briefly if $user is still hydrating to prevent false redirect on refresh
		if (!$user) {
