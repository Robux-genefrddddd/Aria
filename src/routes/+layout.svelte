<script>
	...
	import AdminNotificationModal from '$lib/components/common/AdminNotificationModal.svelte';
	...
	const scheduleDisconnectToast = () => {
		...
		// Silence recurring connection lost toasts in pure frontend mode
	};

	const setupSocket = async (enableWebsocket) => {
		// Pure Firebase mode: safe local event emitter without server connection errors
		const listeners = new Map();
		const mockSocket = {
			id: 'socket-aria-' + Date.now(),
			connected: true,
			on: (event, cb) => {
								...
								if (form_data?.stream ?? false) {
									...
									const processStream = async () => {
										while (true) {
											...
											if ((window as any).__ariaAntiConsoStop) {
	...
	onMount(async () => {
		...
		const handlePageHidden = () => {
			...
			pageWasHidden = true;
			clearDisconnectToastTimer();
			// Mode anti-conso : arrêter la génération si l'onglet est masqué/fermé
			const antiConso = ($settings as any)?.ariaAntiConso ?? true;
			if (antiConso) {
				(window as any).__ariaAntiConsoStop = true;
		...
		const handlePageVisible = () => {
			...
			(window as any).__ariaAntiConsoStop = false;
		...
		const baseThemes = ['dark', 'light', 'oled-dark'];
		let themeCls =
			initialTheme === 'oled-dark' ? 'dark' : initialTheme === 'her' ? 'light' : initialTheme;
		if (initialTheme === 'system') {
			themeCls = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
		}
		baseThemes.filter((e) => e !== themeCls).forEach((e) => document.documentElement.classList.remove(e));
		themeCls.split(' ').forEach((cls) => document.documentElement.classList.add(cls));
		...
		// Ensure splash screen is removed immediately so UI renders smoothly
		setTimeout(() => {
		...
		if (backendConfig) {
			...
			if ($config) {
				...
				try {
					const { onAuthChange, createAriaSessionFromFirebaseUser, subscribeToUserLive } = await import('$lib/firebase');
					onAuthChange(async (firebaseUser) => {
						if (firebaseUser) {
							const sessionUser = await createAriaSessionFromFirebaseUser(firebaseUser);
				...
				if (localStorage.token) {
					// Instant optimistic hydration from localStorage to prevent flash/kickout on refresh
					const rawUser = localStorage.getItem('aria_user');
					if (rawUser && rawUser.trim() !== '') {
						try {
							const cachedUser = JSON.parse(rawUser);
							if (cachedUser) {
								const cachedImg = localStorage.getItem('aria_profile_image_url');
								if (cachedImg && (!cachedUser.profile_image_url || cachedUser.profile_image_url === '/User.avif')) {
									cachedUser.profile_image_url = cachedImg;
								}
								if (cachedUser.id === 'QH8wKG8nWZVtUQEy2pppuBuNZgC3' || cachedUser.email === 'mrpinpinpro@gmail.com') {
									cachedUser.role = 'owner';
								}
								if (cachedUser.token_limit !== undefined && cachedUser.token_limit !== null) {
									cachedUser.token_limit = Number(cachedUser.token_limit);
								}
								await user.set(cachedUser);
							}
						} catch {}
					}

					// Get Session User Info
					let sessionUser = await getSessionUser(localStorage.token).catch((error) => {
						console.warn('Backend user fetch error:', error);
						return null;
					});

					if (!sessionUser && rawUser && rawUser.trim() !== '') {
						try {
					...
					if (sessionUser) {
						if (sessionUser.id === 'QH8wKG8nWZVtUQEy2pppuBuNZgC3' || sessionUser.email === 'mrpinpinpro@gmail.com') {
							sessionUser.role = 'owner';
						}

						// Always fetch latest live role and token_limit directly from Firebase Realtime Database
						try {
							const { getUserRoleFromFirebase, subscribeToUserLive } = await import('$lib/firebase');
							const latestRole = await getUserRoleFromFirebase({
								uid: sessionUser.id || sessionUser.uid,
								email: sessionUser.email
							});
							if (latestRole) {
								sessionUser.role = (sessionUser.id === 'QH8wKG8nWZVtUQEy2pppuBuNZgC3' || sessionUser.email === 'mrpinpinpro@gmail.com') ? 'owner' : latestRole;
							...
							if (fbRes.ok) {
								...
								if (fbData?.role) {
									sessionUser.role = (sessionUser.id === 'QH8wKG8nWZVtUQEy2pppuBuNZgC3' || sessionUser.email === 'mrpinpinpro@gmail.com') ? 'owner' : fbData.role;
									if (sessionUser.role === 'admin' || sessionUser.role === 'owner') {
										sessionUser.permissions = {
											...sessionUser.permissions,
											workspace: { models: true, knowledge: true, prompts: true, tools: true, skills: true },
								...
								if (fbData?.token_limit !== undefined && fbData?.token_limit !== null) {
									sessionUser.token_limit = Number(fbData.token_limit);
								}
								if (fbData?.profile_image_url) {
									sessionUser.profile_image_url = fbData.profile_image_url;
									localStorage.setItem('aria_profile_image_url', fbData.profile_image_url);
								}
								if (fbData?.name) sessionUser.name = fbData.name;
								if (fbData?.email) sessionUser.email = fbData.email;
							}

							// Check dedicated token_limit.json endpoint if not found in root
							if (sessionUser.token_limit === undefined || sessionUser.token_limit === null) {
								try {
									...
									if (tlRes.ok) {
										...
										if (tlData !== null && tlData !== undefined) {
											sessionUser.token_limit = Number(tlData);
										}
									}
								} catch (e) {}
							}
							...
							console.warn('Could not sync latest role/avatar/token_limit:', e);
						}
