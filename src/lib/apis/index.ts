...
export const DEFAULT_BACKEND_CONFIG = {
	...
	default_prompt_suggestions: [
		{
			title: ['Rédiger du code', 'Créer une application web'],
			content: 'Aide-moi à créer une application web moderne avec Svelte et Tailwind.'
		},
		{
			title: ['Analyser des données', 'Extraire les points clés'],
			content: 'Résume et analyse les informations clés de ce document.'
		},
		{
			title: ['Brainstorming', 'Idées créatives'],
			content: 'Génère 10 idées novatrices pour mon nouveau projet.'
		}
	],
	features: {
		auth: false,
		...
		enable_pyodide_file_persistence: true,
...
export const getVersion = async (token: string) => {
	...
	if (error) {
		return { version: '0.11.1', deployment_id: 'local' };
	}

	return res || { version: '0.11.1', deployment_id: 'local' };
};
