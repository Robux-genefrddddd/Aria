					...
					<div class="flex flex-col w-full">
						<div class="flex flex-col w-full mb-2">
							...
							<div class="flex-1 text-xs">
								<a
									href={`/c/${selectedFeedback?.meta?.chat_id || selectedFeedback?.data?.chat_id}`}
									class=" hover:underline"
									target="_blank"
								>
									<span>{selectedFeedback?.meta?.chat_id || selectedFeedback?.data?.chat_id || '-'}</span>
								</a>
							</div>
						</div>

						{#if feedbackData || selectedFeedback}
							{@const fb = feedbackData || selectedFeedback}
							{@const messageId = fb?.meta?.message_id}
							{@const messages = fb?.snapshot?.chat?.chat?.history?.messages || fb?.snapshot?.chat?.history?.messages}

							{#if messages && messageId && messages[messages[messageId]?.parentId]}
								<div class="flex flex-col w-full mb-2">
							...
							{#if messages && messageId && messages[messageId]}
								<div class="flex flex-col w-full mb-2">
						...
						<div class="flex flex-col w-full mb-2">
							...
							<div class="flex-1 text-xs">
								<span>{selectedFeedback?.data?.reason || selectedFeedback?.data?.details?.reason || '-'}</span>
							</div>
						</div>
						...
						<div class="flex flex-col w-full mb-2">
							...
							<div class="flex-1 text-xs">
								<span>{selectedFeedback?.data?.comment || selectedFeedback?.data?.details?.comment || '-'}</span>
							</div>
						</div>
