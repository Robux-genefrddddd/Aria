<script lang="ts">
	...
	const onStopHandler = async (audioBlob, ext: string = 'wav') => {
		...
		if (transcribe) {
			if ($config?.audio?.stt?.engine === 'web' || ($settings?.audio?.stt?.engine ?? '') === 'web' || transcription) {
				onConfirm({
					text: transcription
				});
				confirmed = false;
			...
			try {
				...
				const apiPromise = transcribeAudio(
					localStorage.token,
					file,
			...
			if (res && res.text) {
				...
				// Fallback: send audio file directly so input receives audio attachment without hanging
				onConfirm({
					file: file,
					blob: audioBlob
				});
	...
	const startRecording = async () => {
		...
		if (transcribe) {
			if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
				...
				speechRecognition.lang = $settings?.audio?.stt?.language || 'fr-FR';
				...
				speechRecognition.onresult = async (event: any) => {
					...
					if (cleaned && cleaned.toLowerCase() !== 'ça' && cleaned.toLowerCase() !== 'undefined') {
						transcription = cleaned;
					}
				};

				speechRecognition.onend = function () {
					console.log('Speech recognition ended silently');
				};
