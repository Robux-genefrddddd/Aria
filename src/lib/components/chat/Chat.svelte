<script lang="ts">
	...
	// Read-only check disabled for local mode
	$: readOnly = false;
	...
	const loadChat = async () => {
		...
		await chatId.set(targetId);

		if ($temporaryChatEnabled) {
			temporaryChatEnabled.set(false);
		}

		chat = await getChatById(localStorage.token, targetId).catch(async () => null);

		if (!chat) {
			// Direct fallback to local storage
			try {
				const direct = localStorage.getItem(`aria_chat_${targetId}`);
				if (direct) {
					chat = JSON.parse(direct);
				}
			} catch (e) {}
		}
		...
		if (chat) {
			...
			if (chatContent) {
				...
				history =
					chatContent?.history !== undefined && chatContent?.history?.messages
						? structuredClone(chatContent.history)
						: (chatContent?.messages ? convertMessagesToHistory(chatContent.messages) : { messages: {}, currentId: null });

				if (history && history.messages) {
					const msgKeys = Object.keys(history.messages);
					if (msgKeys.length > 0) {
						if (!history.currentId || !history.messages[history.currentId]) {
							const leaf = Object.values(history.messages).find(
								(m: any) => !m.childrenIds || m.childrenIds.length === 0
							);
							history.currentId = (leaf as any)?.id || msgKeys[msgKeys.length - 1];
						}
					}
				}
				...
				chatTitle.set(chatContent?.title || chat?.title || 'Nouvelle conversation');
	...
	const addMessages = async ({ modelId, parentId, messages }) => {
		...
		try {
			if ($chatId) {
				const firstLine = (inputContent || '').trim().split('\n')[0].slice(0, 60) || 'Nouvelle conversation';
				const { addOrUpdateChatInList } = await import('$lib/stores/chatList');
				addOrUpdateChatInList({
					id: $chatId,
					title: $chatTitle || firstLine,
					updated_at: Math.floor(Date.now() / 1000),
					created_at: Math.floor(Date.now() / 1000)
				});
			}
		} catch (e) {
			console.warn('Sidebar history update error:', e);
		}
	};
	...
	const submitPrompt = async (inputContent, inputFiles) => {
		...
		try {
			const { calculateModelTokenUsage } = await import('$lib/utils/ariaModels');
			const activeModelId = selectedModels?.[0] || 'aria-basic';
			estimatedUsage = calculateModelTokenUsage(activeModelId, inputContent.length, 50);
		} catch (e) {}
		...
		if (res) {
			if (res.choices && res.choices[0]?.message?.content) {
				...
				const timer = setInterval(async () => {
					charIndex = Math.min(fullContent.length, charIndex + Math.floor(Math.random() * 3) + 2);
					responseMessage.content = fullContent.slice(0, charIndex);
					history.messages[responseMessageId] = responseMessage;
					history = history;
					...
					if (charIndex >= fullContent.length) {
						...
						// Update sidebar with the finished chat
						try {
							if ($chatId) {
								const { addOrUpdateChatInList } = await import('$lib/stores/chatList');
								addOrUpdateChatInList({
				...
				// Record weighted token usage based on selected model
				let tokenUsage = Math.ceil(((userMessage?.content?.length || 0) + fullContent.length) / 4);
				const modelIdentifier = typeof model === 'string' ? model : (model?.id || model?.name || 'aria-basic');
				try {
					const { calculateModelTokenUsage } = await import('$lib/utils/ariaModels');
					tokenUsage = calculateModelTokenUsage(
						modelIdentifier,
	...
	const initChatHandler = async (history) => {
		...
		if (!$temporaryChatEnabled) {
			...
			const firstMsg = createMessagesList(history, history.currentId);
			const firstUserMsg = firstMsg.find((m) => m.role === 'user');
			const title = (firstUserMsg?.content || '').trim().split('\n')[0].slice(0, 50) || $i18n.t('New Chat');
			await chatTitle.set(title);
			...
			<div class="w-full h-full flex">
				<div class="h-full flex relative max-w-full min-w-0 flex-1 flex-col">
					...
					<div id="chat-pane" class="flex flex-col flex-auto z-10 w-full @container overflow-auto">
						{#if ($settings?.landingPageMode === 'chat' && !$selectedFolder) || (history?.messages && Object.keys(history.messages).length > 0) || createMessagesList(history, history.currentId).length > 0}
							<div
