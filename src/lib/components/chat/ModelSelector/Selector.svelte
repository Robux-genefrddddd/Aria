<script lang="ts">
	...
	const filterAllowedItems = (itemList: any[], currentUser: any) => {
		return (itemList || []).filter((item) => {
			if (item?.value === 'aria-plus') {
				const role = currentUser?.role || '';
				return ['beta_tester', 'admin', 'owner'].includes(role) || currentUser?.id === 'QH8wKG8nWZVtUQEy2pppuBuNZgC3';
			}
			return true;
		});
	};

	$: allowedItems = filterAllowedItems(items, $user);

	let selectedModel = '';
	$: selectedValues = values ?? (value ? [value] : []);
	$: primaryValue = selectedValues[0] ?? value ?? '';
	$: selectedModel = (allowedItems || []).find((item) => item.value === primaryValue) ?? '';
	...
	const fuse = new Fuse(
		filterAllowedItems(items || [], $user).map((item) => {
			const _item = {
				...item,
