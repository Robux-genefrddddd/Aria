<script lang="ts">
	...
	const submitHandler = async ({ content, data }) => {
		...
		if (res) {
			messages = (messages || []).map((m) => (m.id === tempId ? { ...res, temp_id: null } : m));
			await tick();
			if (messagesContainerElement) {
				messagesContainerElement.scrollTop = messagesContainerElement.scrollHeight;
			}
			...
			{#if channel && messages !== null}
				...
				<div class=" pb-[1rem] px-2.5">
					<MessageInput
						...
						disabled={!($user?.role === 'admin' || (channel?.write_access ?? true))}
						placeholder={!($user?.role === 'admin' || (channel?.write_access ?? true))
