<script lang="ts">
	...
	import TokenUsageMeter from '$lib/components/common/TokenUsageMeter.svelte';
	...
	const newChatHandler = async () => {
		...
		// Dispatch custom event to reset Chat component immediately
		if (typeof window !== 'undefined') {
			window.dispatchEvent(new CustomEvent('init-new-chat'));
		}
						...
						{#if $user !== undefined && $user !== null}
							<UserMenu role={$user?.role} profile={$config?.features?.enable_user_status ?? true}>
								...
								>
									...
									>
										<img
											src={$user?.profile_image_url || '/User.avif'}
							...
							>
								...
								>
									<div class=" self-center mr-3 relative flex-shrink-0">
										<img
											src={$user?.profile_image_url || (typeof window !== 'undefined' ? localStorage.getItem('aria_profile_image_url') : null) || '/User.avif'}
											class="size-5.5 object-cover rounded-full"
											alt={$i18n.t('Open User Profile Menu')}
											aria-label={$i18n.t('Open User Profile Menu')}
											on:error={(e) => {
												const cached = typeof window !== 'undefined' ? localStorage.getItem('aria_profile_image_url') : null;
