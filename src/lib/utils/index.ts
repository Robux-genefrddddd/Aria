...
export const processResponseContent = (content: string) => {
	...
	// Convert <think>...</think> tags into a clean interactive collapsible Réflexion box
	if (content.includes('<think>')) {
		if (content.includes('</think>')) {
			content = content.replace(
				/<think>([\s\S]*?)<\/think>/gi,
				(match, thoughtContent) => {
					const clean = thoughtContent.trim();
					if (!clean) return '';
					return `\n\n<details class="thought-box mb-3 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/40 p-3 text-xs text-gray-600 dark:text-gray-400 font-sans"><summary class="cursor-pointer font-medium text-gray-700 dark:text-gray-300 select-none flex items-center gap-1.5 list-none hover:text-gray-900 dark:hover:text-white transition">🧠 <span>Réflexion & Analyse de l'IA</span></summary><div class="mt-2.5 pt-2.5 border-t border-gray-200/60 dark:border-gray-800/60 text-gray-600 dark:text-gray-300 leading-relaxed font-mono whitespace-pre-wrap">${clean}</div></details>\n\n`;
				}
			);
		} else {
			// Streaming in progress: <think> without closing tag yet
			content = content.replace(
				/<think>([\s\S]*)$/gi,
				(match, thoughtContent) => {
					const clean = thoughtContent.trim();
					return `\n\n<details open class="thought-box mb-3 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/40 p-3 text-xs text-gray-600 dark:text-gray-400 font-sans"><summary class="cursor-pointer font-medium text-gray-700 dark:text-gray-300 select-none flex items-center gap-1.5 list-none hover:text-gray-900 dark:hover:text-white transition animate-pulse">🧠 <span>En cours de réflexion...</span></summary><div class="mt-2.5 pt-2.5 border-t border-gray-200/60 dark:border-gray-800/60 text-gray-600 dark:text-gray-300 leading-relaxed font-mono whitespace-pre-wrap">${clean}</div></details>\n\n`;
				}
			);
		}
	}
...
export const getTimeRange = (timestamp) => {
	...
	const tsNum = typeof timestamp === 'string' ? parseInt(timestamp) : Number(timestamp);
	if (!tsNum || isNaN(tsNum)) return 'Today';

	const now = new Date();
	const date = new Date(tsNum < 1_000_000_000_000 ? tsNum * 1000 : tsNum);
...
export const createMessagesList = (history, messageId) => {
	...
	const list = [];
	let currentId = messageId || history.currentId;

	if (!currentId || !history.messages[currentId]) {
		const keys = Object.keys(history.messages);
		if (keys.length > 0) {
	...
	if (list.length === 0 && Object.keys(history.messages).length > 0) {
